app.controller("trainingController",["$scope", "dataService", "$rootScope", "$state", "$filter", function($scope, dataService, $rootScope, $state, $filter){
    ...
    $scope.getUserTalks = function () {
        $scope.index = 0;
        var webURL = 'api/gynac/getuserbookmark?userId=' + $scope.userId + '&&talkId= 0';
        dataService.getData(webURL, {}).then(function (data) {           
            $scope.userTalks = data;
            console.log(data);
    ...
    $scope.getBookmark = function () {
        debugger;
        console.log($scope.selectuserTalk);
        var webURL = 'api/gynac/getuserbookmark?userId=' + $scope.userId + '&&talkId=' + $scope.selectuserTalk;
        dataService.getData(webURL, {}).then(function (data) {
